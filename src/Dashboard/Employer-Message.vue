<template>
  <IonGrid style="height: 100%; width: 100%; padding: 0">
    <IonRow style="height: 100%; width: 100%">
      <IonCol class="emessage-container" size="3.5">
        <IonRow>
          <IonCol class="flexcenter" style="justify-content: left">
            <div>
              <IonText
                class="emessage-tile flexcenter"
                style="justify-content: left"
              >
                <IonIcon style="margin: 5px" :icon="mail"> </IonIcon>
                INBOX
              </IonText>
            </div>
            <div class="flexcenter" style="position: absolute; right: 0">
              <IonButton
                class="emessage-button-markasdone"
                v-if="markasDone == true"
                @click="swapViews(false)"
              >
                <IonIcon :icon="checkmark"></IonIcon>
              </IonButton>
              <IonButton
                class="emessage-button-markasdone"
                v-else-if="markasDone == false"
                @click="swapViews(true)"
              >
          ...
          <IonCol class="custom-scrollbar">
            <div v-for="chat in filteredSearch">
              <div
                @click="
                  clickUser(
                    chat.data.SeekerName,
                    chat.data.SeekerEmail,
                    chat.data.JobSwipe,
                    chat.data.SeekerPicture,
                    chat.id
                  )
                "
            ...
            <div class="flexcenter">
              ...
              >
                ...
                <IonFabList side="bottom" class="flexcenter">
                  <IonFabButton
                    @click="markDoneUndone(false)"
                    class="emessage-fab-button-contents"
                  >
                    <IonIcon
              ...
              <IonFab
                v-if="clickedChat && markasDone == false"
                style="position: absolute; right: 5px"
              >
                <IonFabButton class="emessage-fab-button">
                  <IonIcon :icon="chevronDown"></IonIcon>
                ...
                <IonFabList side="bottom" class="flexcenter">
                  <IonFabButton
                    @click="markDoneUndone(true)"
                    class="emessage-fab-button-contents2"
                  >
                  ...
                  <IonFabButton
                    @click="deleteChats()"
                    class="emessage-fab-button-contents3"
                  >
            ...
            >
              <div v-if="messages.length != 0">
                <div v-for="(message, index) in messages">
                  ...
                  >
                    <div
                      class="emessage-sender emessage-removemessage-container-parent"
                    >
                      <div
                        id="removeButton"
                        @click="confirmRemoveMessage(message.id)"
                      ...
                      <div>
                        <div class="emessage-sender">
                          <img
                            @click="
                              openModalviewpicture(true, message.messagePicture)
                            "
                            v-if="message.messagePicture"
                            class="emessage-pictureSent"
                            :src="
                              message
                        ...
                        >
                          <a
                            ...
                            "
                          >
                            <IonCard class="eprofile-card-messages">
                              {{
                                messages ? `${message.messagePdfname}` : "..."
                              }}
                            </IonCard>
                    ...
                    <div class="emessage-textdatesent">
                      <IonText class="emessage-textdatesent">{{
                  ...
                  >
                    <div class="emessage-receiver">
                      ...
                      <div>
                        ...
                        <div class="emessage-receiver">
                          <IonCard
                            v-if="message.messageText"
                            class="eprofile-card-messages2"
                          >
                            {{ messages ? `${message.messageText}` : "..." }}
                          </IonCard>
                        </div>
                        <div
                          class="emessage-receiver"
                          v-if="messages ? `${message.messagePdf}` : '...'"
                        >
                          <a
                            :href="messages ? `${message.messagePdf}` : '...'"
                            ...
                            "
                          >
                            <IonCard class="eprofile-card-messages2">
                              {{
                                messages ? `${message.messagePdfname}` : "..."
                              }}
                            </IonCard>
                    ...
                    <div
                      class="emessage-textdatesent2"
                      style="margin: 0 0 0 10px"
                    >
                      <IonText class="emessage-textdatesent2">{{
                        formatTimestamp(message.dateSent)
                      }}</IonText>
                    </div>
                  </div>
  ...
  >
    <img
      class="emessage-pictureSentFull"
      :src="setviewPicturefull"
      alt="Selected Image"
    />
  </IonModal>
</template>
  ...
  methods: {
    ...
    async getChats() {
      const q = query(
        ...
        where("EmployerEmail", "==", this.sender),
        where("EmployerActive", "==", this.markasDone),
